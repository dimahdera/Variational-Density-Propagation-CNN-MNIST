...
def Model_with_uncertainty_computation(x, conv1_weight_M, conv1_weight_sigma, fc1_weight_mu, fc1_bias_mu,fc1_weight_sigma,
    ...
    x_train_patches = tf.extract_image_patches(x, ksizes=[1, patch_size, patch_size, 1], strides=[1,1,1,1], rates=[1,1,1,1], padding = "VALID")# shape=[1, 24, 24, 25 ]
    ...
    argmax2 = tf.reshape(argmax1,[1, num_filters[0], -1])#shape=[1, 32, 144]
    new_sigma_g= tf.transpose(sigma_g, [2, 0, 1]) # shape =[32, 24*24, 24*24]=[32, 576, 576]
    ...
    index = tf.squeeze(index) # shape=[32,144]      
    for i in range(num_filters[0]):
        if(i==0):
            ...
            new_ind = tf.concat([new_ind,ind1],0) # shape=[32*12*12]= [32*144] 
    down_sample1 = tf.gather(new_sigma_g,new_ind) 
    down_sample1 = tf.reshape(down_sample1, [num_filters[0], new_size*new_size, -1])
    down_sample1 = tf.transpose(down_sample1, [0, 2, 1]) 
    down_sample1 = tf.reshape(down_sample1, [num_filters[0]*image_size*image_size, -1])
    down_sample2 = tf.gather(down_sample1,new_ind)
    sigma_p = tf.reshape(down_sample2,[num_filters[0],new_size*new_size,new_size*new_size]) #shape=[32,144, 144] 
    ######################################################
    ######################################################
    ...
    # Propagate the mean
    mu_y = tf.nn.softmax(mu_f) #shape=[1, 10] 
    # Propagate the covariance matrix
    # compute the gradient of softmax manually  
    grad_f1 = tf.matmul(tf.transpose(mu_y), mu_y)  
    diag_f = tf.diag(tf.squeeze(mu_y))
    grad_soft = diag_f - grad_f1 #shape=[10,10]
    sigma_y = tf.matmul(grad_soft,   tf.matmul(sigma_f, tf.transpose(grad_soft)))#shape=[10,10] 
...
def main_function(image_size=28, num_channel=1, patch_size=5, num_filters=[32],num_labels=10, batch_size = 50, noise_limit = 0.1,
         noise_l2_weight = 0.01, adversary_target_cls=3, Adv_lr = 0.8e-4, lr = 1e-4, init_sigma_std=-2.2 , init_mu=0.1, epochs =10,
